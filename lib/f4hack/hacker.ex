defmodule F4Hack.Hacker do
  ...
  def handle_call({:words, words}, _from, state) do
    ...
    if Enum.all?(word_list, fn(w) -> String.length(w) == word_length end) do
      {:reply, remaining_words(word_list), %{tries: 0, guess: nil, words: word_list, length: word_length}}
    else
      {:stop, :normal, :error_unequal_length, state}
    end
  end
  ...
  def handle_call({:likeness, _}, _from, state = %{tries: 4}) do
  ...
  def handle_call({:likeness, likeness}, _from, state = %{tries: tries, guess: guess, words: word_list}) do
    ...
    case length(new_word_list) do
      ...
      _ -> {:reply, remaining_words(new_word_list), %{state | tries: tries + 1, guess: nil, words: new_word_list}}
    end
  end
  ...
  defp remaining_words(word_list) do
    {:remaining_words, Enum.join(word_list, " ")}
  end
